...
const srcDir = 'c:/Users/jenki/Desktop/My Coding Projects/Tavern of Holding/tavern-of-holding/src';
...
function walkDir(dir) {
    ...
    for (const file of files) {
        ...
        if (stat.isDirectory()) {
            walkDir(fullPath);
        } else if (fullPath.endsWith('.svelte') || fullPath.endsWith('.ts') || fullPath.endsWith('.js') || fullPath.endsWith('.html') || fullPath.endsWith('.css')) {
            ...
            for (const [hex, cssVar] of Object.entries(replacements)) {
                ...
                const regexUpper = new RegExp(escapedHex.toUpperCase(), 'g');
